            ...
            try:
                for i in range(len(new_str)):
                    ...
                    if ord(new_str[i])<=ord(str2[0]) and ord(new_str[i+1])>ord(str2[0]):
                        ...
                        break
                    elif ord(new_str[i]) > ord(str2[0]):
                        return(print(str2+new_str))
            except:
                return(print(new_str + str2))
        else:
            try:
                yos = new_str[:new_str.index(str2[0])] + str2 + new_str[new_str.index(str2[0]):]
                POS = new_str[:new_str.index(str2[0]) + 1] + str2 + new_str[new_str.index(str2[0]) + 1:]

                for i in range(len(new_str)):
                    if ord(new_str[i])<=ord(str2[0]) and ord(new_str[i+1])>ord(str2[0]):
                        return(print(min(new_str[:i+1] + str2 + new_str[i+1:],yos,POS)))
                        break
                    elif ord(new_str[i]) == ord(str2[0]) and ord(new_str[i + 1]) > ord(str2[0]):
                        yos2 = str2+new_str
                        pos2 = new_str[:i]+str2+new_str[i:]
                        return (print(min(new_str[:i + 1] + str2 + new_str[i + 1:], yos2, pos2)))
                        break
            except:
                return(print(min(new_str + str2,yos,POS)))
